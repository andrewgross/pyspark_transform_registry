...
class TestFunctionLoading:
    ...
    def test_load_registered_function(self, spark, mlflow_tracking):
        ...
        # Load function
        loaded_func = load_function("test.load.transform")
...
class TestModelListing:
    ...
    def test_list_registered_functions(self, spark, mlflow_tracking):
        """Test listing registered functions."""

        def test_func1(df: DataFrame) -> DataFrame:
            return df.select("*")

        def test_func2(df: DataFrame) -> DataFrame:
            return df.select("*")
        ...
        register_function(
            func=test_func2,
            name="test.list.func2",
            input_example=test_data,
            description="Test function 2",
        )

        # List all functions
        all_functions = list_registered_functions()
        assert len(all_functions) >= 2

        # List with prefix
        test_functions = list_registered_functions("test.list.")
        assert len(test_functions) >= 2

        # Check structure
        for func_info in test_functions:
            assert "name" in func_info
            assert "latest_version" in func_info
            assert "description" in func_info
            assert "tags" in func_info
